...
from bilby.core.utils import log as bilby_log
try:
    bilby_log.env_package_list = lambda as_dataframe=False: None
except Exception:
    pass
...
def dimention1to2(ij_input):
    ij = ij_input-1
    temp = int(np.sqrt(ij)) ## temp represents the largest number in the group
    #part = temp*2+1
    #n = ij - temp**2 +1
...
#############################################################  ploting end #####################################################################
    ...
    rows = [duration, lh, indexl, indexh, logb, maxloglikelihood,rho_inj_m,rho_inj_g,rho_optcomb_m,rho_optcomb_g, sampling_time, total_croptime_l, total_croptime_h ,total_notchlist_l, total_notchlist_h]
