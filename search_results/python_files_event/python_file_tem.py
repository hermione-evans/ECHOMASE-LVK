...
chi = 0.67 # for GW150914 and GW231226, the final spin is both approximately 0.67
...
if eventname == 'GW150914':
    ...
    gps = 1126259462.4 + 0.04 # the main event time
    ...
    t0 = 0 # in unit of second. for event search, we set t0=0
...
if eventname == 'GW250114':
    ...
    notchout_amplitude_list = [6] #The normalized strain data in frequency domain higher than this parameter will be outched out.
    croptime = 1 # in unit of second
...
## directory
label0 = eventname+'_npoint={0:1d}'.format(npoints)+'_duration={:.0f}'.format(duration)+'_'+likelihood_string
outdirtemp = 'outdir_'+eventname+'_event_version3.5_psd0.5_rwalk'
outdir = '../'+outdirtemp+'/'+label0

if(whether_print_header == 0):
    headers = ['duration', 'logB', 'maxloglikelihood', 'SNR_comb_median', 'SNR_comb_global', 'run_time',
...
strainLp = noiseLp
strainHp = noiseHp
print('gps of the segment start time of the main event:', strainL.t0.value)
...
# if notch_index == 1:#notch
rows = [duration, logb, maxloglikelihood,rho_optcomb_m,rho_optcomb_g, sampling_time, total_croptime_l, total_croptime_h ,total_notchlist_l, total_notchlist_h]
# elif notch_index == 0:#nonotch
